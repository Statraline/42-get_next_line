...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/04 15:21:07 by srichard          #+#    #+#             */
/*   Updated: 2025/12/04 15:21:11 by srichard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	line_buffer = read_and_stash(fd, stash[fd], buffer);
	free(buffer);
	buffer = NULL;
	if (!line_buffer)
		return (NULL);
	stash[fd] = extract_line(line_buffer);
	return (line_buffer);
}
